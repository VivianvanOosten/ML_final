...
recommendation_model$LIBMF_realRatingMatrix$parameters
```
...
sampled_data<- sample(x = c(TRUE, FALSE),
                      ...
                      replace = TRUE,
                      prob = c(0.8, 0.2)) 
training_data <- df_sparse[sampled_data, ]
testing_data <- df_sparse[!sampled_data, ]
...
parameters <-c(
  'IBCF' = list(k = 35),
  'UBCF' = list(method="Cosine", nn=10),
  "LIBMF" = NULL
)

get_RMSE <- function(grid_row, df){
  print(grid_row)
  
  df_sparse <- selecting_fav_movies(df_sparse, grid_row[m], grid_row[n])
  #df_sparse <- as.matrix(df_sparse) 
  df_sparse <- as(df_sparse, "realRatingMatrix")
  #normalized_ratings <- normalize(df_sparse)
  ...
  recommen_model <- Recommender(data = training_data,
                                   method = grid_row[modelName],
                                   parameter = parameters[grid_row[modelName]])
...
set.seed(42)
  ...
  e <- evaluationScheme(df_sparse, method="split", train=0.8, given=-5)
...
recommen_model_UBCF <- Recommender(data = training_data,
                                   method = 'UBCF',
