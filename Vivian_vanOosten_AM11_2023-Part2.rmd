...
DTM_tidy %>%
  ...
  arrange(-wordCount) %>%
  head(10)
# most used words are comedi and end
```
...
Most words have a word-count bewteen 0 and 2500. It's likely that these words will hold the most information. However, some words, such as 'great', have been used very frequently and can also be very informative for the rating of that movie. Therefore, we do not want to use TF-IDF to weight the words with. 
...
Clearly the additional relevance of columns decreases very quickly. Below we take a closer look at the plot to determine the exact dimensions we want to take. 
...
Based on this plot, we will keep 25 columns, since the curve levels off after that. This will explain 69% of the variance in the data. This is well over half, which is very good given taht we went from 699 variables to 25. 
