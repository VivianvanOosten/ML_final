...
# converting the DTM into a tibble for tidyverse to work with
m    <- as.matrix(DTM) # DTM is a list, convert it to matrix
M    <- cbind(movieIds, m) # bind column called movieId, and other columns in matrix into a single matrix M
DTM_tbl <- as_tibble(M) # transform matrix into tibble (which is what tidyverse works with)
DTM_tbl_tidy <- pivot_longer(DTM_tbl, cols = !movieIds, names_to = "word", values_to = "wordCount")
```
...
# not sure about PCA
DTM_tbl <- scale(DTM_tbl, center = TRUE, scale = TRUE)
